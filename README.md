...
- **声明知识范围**：开讲前 disclose 自己知道什么 / 不知道什么（尤其是时效性 / 特定来源的内容），不够时让学习者提供权威源或主动去抓。
- **三档强度**：quick / standard / deep，按问题复杂度选择，默认从轻量开始。
- **一次只问一个问题**：提问后停下等待真实回答，不自问自答，也不一次性倒出问题清单。
- **先调查真实上下文**：讲真实仓库或系统时，先读代码、README、架构文档或日志再设计问题。
- **逐步撤除脚手架**：从提示、类比、部分示例开始，随着学习者理解加深逐步放手。
- **图表优先**：分类、对比、流程、层级、象限——优先用表格或 ASCII 图，别只用文字。
- **代码双语对照**：学习者有熟练的"参考语言"（如 Go）时，目标语言代码同时给参考语言对照版，方便用熟悉的语言推断陌生的。
- **多次会话用三层 retrieval**：日级 mini check（5 分钟）+ 周级复盘 + 阶段级全套测验。仅靠"验证迁移"对抗不了遗忘曲线；主动回想比再读一遍有效得多。**< 80% 正确率当天回炉**。
- **应试场景按权重分级**：先定"通过线 vs 精通"目标；过线导向时按 T0–T3 给考点分级，集中攻高频高分（T0/T1），低频偏门（T3）战略放弃。开讲前先 WebSearch 真题分布。
- **错误模式标签化**：错题打标签（概念混淆 / 单位数字 / 原文不熟 / 计算粗心 / 范围漏边），累积后周复盘优先攻占比最高的一类。
- **进度可视化**：长跑学习每次开学前给一张进度卡（markdown 表格 + 纯 ASCII 进度条），一眼看到离目标多远、哪块是命脉、节奏超前还是落后。
